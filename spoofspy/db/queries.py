...
trust_aggregate = text(
    """SELECT 
    game_server_address, array_agg(game_server_port), array_agg(last_week_avg_trust_score)
    FROM (
        SELECT
        game_server_address, game_server_port, avg(avg_trust_score) AS last_week_avg_trust_score
